function(page, callback){
  ...
  if(key==='%'+'GOOGLEAPIKEY_PSI_NEW%'){
    callback(that.createResult('SPEED', '"Page Speed Insights v5 FCP and FID desktop" rule not yet enabled! Set <a href="https://developers.google.com/speed/docs/insights/v5/get-started" target="_blank">Google API Key</a> in <a href="'+that.getGlobals().rulesUrl+'" target="_blank">Settings</a>.', "warning"));
    return;
  }
  ...
  .then(
    function(response) {
      if (response.status !== 200) {
        callback(that.createResult('SPEED', 'No Page Speed Insights v5 FCP and FID desktop data. (Response Status '+response.status+' '+response.text+')', "warning"));
        return;
      }
      response.json().then(function(data) {
        if (data.loadingExperience.hasOwnProperty('metrics')){
          ...
          callback(that.createResult('SPEED', 'No Page Speed Insights v5 FCP and FID desktop data.', "warning"));
          return;
        }
  ...
  .catch(function(err) {
    callback(that.createResult('SPEED', 'No Page Speed Insights v5 FCP and FID <b>desktop</b> data. '+err+' <a href="https://developers.google.com/speed/pagespeed/insights/?hl=en&url='+url+'" target="_blank">Page Speed Insights</a>', "warning"));
  });
